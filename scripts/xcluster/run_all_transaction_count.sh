...
usage() {
  cat <<'EOF'
Usage: bash scripts/xcluster/run_all_transaction_count.sh [options]

Runs XCluster transaction count configs (plain, index, fk, index_fk). Collects replication lag.

Options:
  --run-root PATH          Run set folder (default: runs/xcluster_<timestamp>_transaction_count)
...
[[ -z "$run_root" ]] && run_root="runs/xcluster_$(date +%Y%m%d-%H%M%S)_transaction_count"
...
for cfg in config/xcluster/transaction_count/plain.json config/xcluster/transaction_count/index.json config/xcluster/transaction_count/fk.json config/xcluster/transaction_count/index_fk.json; do
  echo "Running: $cfg"
  python3 -m ysqlload.cli --config "$cfg" --run-root "$run_root"
done

echo "XCluster transaction count run set completed: $run_root"

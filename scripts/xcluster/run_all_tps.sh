...
usage() {
  cat <<'EOF'
Usage: bash scripts/xcluster/run_all_tps.sh [options]

Runs XCluster TPS configs (plain, index, fk, index_fk). Collects replication lag during load.

Options:
  --run-root PATH          Run set folder (default: runs/xcluster_<timestamp>_tps)
...
for cfg in config/xcluster/tps/plain.json config/xcluster/tps/index.json config/xcluster/tps/fk.json config/xcluster/tps/index_fk.json; do
  echo "Running: $cfg"
  python3 -m ysqlload.cli --config "$cfg" --run-root "$run_root"
done

echo "XCluster TPS run set completed: $run_root"

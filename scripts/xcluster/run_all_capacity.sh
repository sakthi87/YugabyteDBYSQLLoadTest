...
usage() {
  cat <<'EOF'
Usage: bash scripts/xcluster/run_all_capacity.sh [options]

Runs XCluster capacity configs (plain, index, fk, index_fk). Collects replication lag.

Options:
  --run-root PATH          Run set folder (default: runs/xcluster_<timestamp>_capacity)
...
[[ -z "$run_root" ]] && run_root="runs/xcluster_$(date +%Y%m%d-%H%M%S)_capacity"
...
for cfg in config/xcluster/capacity/plain.json config/xcluster/capacity/index.json config/xcluster/capacity/fk.json config/xcluster/capacity/index_fk.json; do
  echo "Running: $cfg"
  python3 -m ysqlload.cli --config "$cfg" --run-root "$run_root"
done

echo "XCluster capacity run set completed: $run_root"

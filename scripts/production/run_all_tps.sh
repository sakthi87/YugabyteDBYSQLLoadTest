...
usage() {
  cat <<'EOF'
Usage: bash scripts/production/run_all_tps.sh [options]

Runs production TPS configs (plain, index, fk, index_fk) - 1K/2K/5K/8K/10K TPS with scaled clients.
...
[[ -z "$run_root" ]] && run_root="runs/$(date +%Y%m%d-%H%M%S)_production_tps"
mkdir -p "$run_root"
...
for cfg in config/production/tps/plain.json config/production/tps/index.json config/production/tps/fk.json config/production/tps/index_fk.json; do
  echo "Running: $cfg"

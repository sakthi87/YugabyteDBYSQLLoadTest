...
usage() {
  cat <<'EOF'
Usage: bash scripts/production/run_all_transaction_count.sh [options]

Runs production transaction count configs (plain, index, fk, index_fk) - fixed 50K/100K/200K tx.

Options:
  --run-root PATH          Run set folder (default: runs/<timestamp>_production_transaction_count)
...
[[ -z "$run_root" ]] && run_root="runs/$(date +%Y%m%d-%H%M%S)_production_transaction_count"
mkdir -p "$run_root"
...
for cfg in config/production/transaction_count/plain.json config/production/transaction_count/index.json config/production/transaction_count/fk.json config/production/transaction_count/index_fk.json; do
  echo "Running: $cfg"

...
usage() {
  cat <<'EOF'
Usage: bash scripts/production/run_all_capacity.sh [options]

Runs production capacity configs (plain, index, fk, index_fk) - max throughput 60/120 sec, scaled clients.

Options:
  --run-root PATH          Run set folder (default: runs/<timestamp>_production_capacity)
...
[[ -z "$run_root" ]] && run_root="runs/$(date +%Y%m%d-%H%M%S)_production_capacity"
mkdir -p "$run_root"
...
for cfg in config/production/capacity/plain.json config/production/capacity/index.json config/production/capacity/fk.json config/production/capacity/index_fk.json; do
  echo "Running: $cfg"

...
usage() {
  cat <<'EOF'
Usage: bash scripts/stretch/run_all_capacity.sh [options]

Runs Stretch Cluster capacity configs (plain, index, fk, index_fk).

Options:
  --run-root PATH          Run set folder (default: runs/stretch_<timestamp>_capacity)
...
[[ -z "$run_root" ]] && run_root="runs/stretch_$(date +%Y%m%d-%H%M%S)_capacity"
mkdir -p "$run_root"
...
for cfg in config/stretch/capacity/plain.json config/stretch/capacity/index.json config/stretch/capacity/fk.json config/stretch/capacity/index_fk.json; do
  echo "Running: $cfg"
  python3 -m ysqlload.cli --config "$cfg" --run-root "$run_root"
done

echo "Stretch capacity run set completed: $run_root"

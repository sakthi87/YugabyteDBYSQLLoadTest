...
usage() {
  cat <<'EOF'
Usage: bash scripts/stretch/run_all_tps.sh [options]

Runs Stretch Cluster TPS configs (plain, index, fk, index_fk).

Options:
  --run-root PATH          Run set folder (default: runs/stretch_<timestamp>_tps)
...
[[ -z "$run_root" ]] && run_root="runs/stretch_$(date +%Y%m%d-%H%M%S)_tps"
mkdir -p "$run_root"
...
for cfg in config/stretch/tps/plain.json config/stretch/tps/index.json config/stretch/tps/fk.json config/stretch/tps/index_fk.json; do
  echo "Running: $cfg"
  python3 -m ysqlload.cli --config "$cfg" --run-root "$run_root"
done

echo "Stretch TPS run set completed: $run_root"

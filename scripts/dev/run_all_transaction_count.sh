...
usage() {
  cat <<'EOF'
Usage: bash scripts/dev/run_all_transaction_count.sh [options]

Runs dev transaction count configs (plain, index, fk, index_fk) - fixed 10K/20K/50K tx.

Options:
  --run-root PATH          Run set folder (default: runs/<timestamp>_dev_transaction_count)
...
[[ -z "$run_root" ]] && run_root="runs/$(date +%Y%m%d-%H%M%S)_dev_transaction_count"
mkdir -p "$run_root"
...
for cfg in config/dev/transaction_count/plain.json config/dev/transaction_count/index.json config/dev/transaction_count/fk.json config/dev/transaction_count/index_fk.json; do
  echo "Running: $cfg"

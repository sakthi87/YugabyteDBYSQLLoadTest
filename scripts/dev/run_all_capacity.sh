...
usage() {
  cat <<'EOF'
Usage: bash scripts/dev/run_all_capacity.sh [options]

Runs dev capacity configs (plain, index, fk, index_fk) - max throughput for 60/120 sec.

Options:
  --run-root PATH          Run set folder (default: runs/<timestamp>_dev_capacity)
...
[[ -z "$run_root" ]] && run_root="runs/$(date +%Y%m%d-%H%M%S)_dev_capacity"
mkdir -p "$run_root"
...
for cfg in config/dev/capacity/plain.json config/dev/capacity/index.json config/dev/capacity/fk.json config/dev/capacity/index_fk.json; do
  echo "Running: $cfg"

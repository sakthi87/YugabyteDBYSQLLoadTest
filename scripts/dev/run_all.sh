...
usage() {
  cat <<'EOF'
Usage: bash scripts/dev/run_all.sh [options]

Runs all dev configs: TPS + transaction count + capacity (12 scenarios total).

Options:
  --run-root PATH          Run set folder (default: runs/<timestamp>_dev_all)
...
[[ -z "$run_root" ]] && run_root="runs/$(date +%Y%m%d-%H%M%S)_dev_all"
mkdir -p "$run_root"
...
configs=(
  config/dev/tps/plain.json config/dev/tps/index.json config/dev/tps/fk.json config/dev/tps/index_fk.json
  config/dev/transaction_count/plain.json config/dev/transaction_count/index.json config/dev/transaction_count/fk.json config/dev/transaction_count/index_fk.json
  config/dev/capacity/plain.json config/dev/capacity/index.json config/dev/capacity/fk.json config/dev/capacity/index_fk.json
)
